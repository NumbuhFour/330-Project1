...
app.player = {
	...
	drawPlanetIndicator: function(ctx, planet){
		...
		var distDisplay = dist;
		
		if(dist > 1000) distDisplay = (dist/1000).toFixed(2) + "k"
		...
		if(dist < 1000){
			var scale = Math.max(dist/1000, 0.5);
		...
		this.drawLib.text(ctx,distDisplay + "m",0,0,12,planet.color);
		ctx.restore();
